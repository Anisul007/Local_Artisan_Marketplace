...
export default function AdminSettingsPage() {
  ...
  if (!settings) return <div className="h-24 animate-pulse rounded-xl bg-white/10" />;
    ...
    <div className="rounded-2xl border border-white/15 bg-slate-900/70 p-5 shadow-xl shadow-black/25">
      ...
      <label className="mt-3 block text-sm font-semibold text-slate-200">
        ...
        <textarea
          ...
          rows={3}
          className="mt-1 w-full rounded-lg border border-white/20 bg-white/5 p-2 text-sm text-slate-100"
        />
      </label>
      <button onClick={save} className="mt-4 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white">
        Save settings
      </button>
    </div>
  );
...
function Field({ label, value, onChange }) {
  return (
    <label className="block text-sm font-semibold text-slate-200">
      {label}
      <input value={value} onChange={(e) => onChange(e.target.value)} className="mt-1 h-10 w-full rounded-lg border border-white/20 bg-white/5 px-3 text-sm text-slate-100" />
    </label>
  );
}
