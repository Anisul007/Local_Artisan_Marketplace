    ...
    <main className="max-w-4xl mx-auto px-4 py-8 md:py-12">
      ...
      {purchaseBlocked && (
        <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          <p className="font-semibold">Purchasing is for customers only</p>
          <p className="mt-1 text-amber-800/90">{vendorPurchaseMessage}</p>
          <p className="mt-2">
            <Link to="/login" className="font-semibold text-purple-700 hover:underline">Sign in as a customer</Link>
            {" · "}
            <Link to="/register?role=customer" className="font-semibold text-purple-700 hover:underline">Create a customer account</Link>
          </p>
        </div>
      )}
      ...
      <div className="mt-8 p-6 rounded-xl border border-gray-200 bg-gray-50">
        {!coupon ? (
          <div className="mb-4 flex flex-wrap gap-2">
            <input
              ...
              disabled={purchaseBlocked}
              className="flex-1 min-w-[120px] rounded-lg border border-gray-300 px-3 py-2 text-sm uppercase placeholder:normal-case disabled:bg-gray-100 disabled:opacity-60"
            />
            <button
              type="button"
              disabled={purchaseBlocked || couponLoading || !codeInput.trim()}
              onClick={async () => {
                setCouponError("");
        ...
        {purchaseBlocked ? (
          <p className="mt-4 w-full text-center rounded-xl border border-gray-300 bg-gray-100 py-3 text-sm font-medium text-gray-600">
