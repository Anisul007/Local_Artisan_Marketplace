...
export default function VendorReportAbusePage() {
  ...
  if (!user) return <Navigate to={`/login?next=/vendor/report-abuse${window.location.search}`} replace />;
  if (user.role !== "vendor") return <Navigate to={`/account/report-abuse${window.location.search}`} replace />;

  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900">Report abuse or a policy issue</h1>
      <p className="mt-1 text-gray-600">Use this when a customer, order, or marketplace listing violates platform rules.</p>
