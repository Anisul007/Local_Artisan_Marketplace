...
router.use((err, _req, res, next) => {
  if (err instanceof multer.MulterError) {
    if (err.code === "LIMIT_FILE_SIZE") {
      return res.status(400).json({ ok: false, message: "Logo too large. Max 2MB." });
    }
    return res.status(400).json({ ok: false, message: err.message || "Logo upload failed" });
  }
  if (err?.message?.includes("Only JPEG/PNG allowed")) {
    return res.status(400).json({ ok: false, message: "Only JPEG/PNG allowed" });
  }
  return next(err);
});
